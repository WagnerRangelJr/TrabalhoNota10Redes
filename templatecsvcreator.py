...
('Numero de bits transmitidos: {0:d}'.format(reps * packetLength * 8))
('Numero de bits errados inseridos: {0:d}\n'.format(totalInsertedErrorCount))
('Taxa de erro de bits (antes da decodificacao): {0:.2f}%'.format(
    float(totalInsertedErrorCount) / float(reps * len(codedPacket)) * 100.0))
('Numero de bits corrompidos apos decodificacao: {0:d}'.format(totalBitErrorCount))
('Taxa de erro de bits (apos decodificacao): {0:.2f}%\n'.format(
    float(totalBitErrorCount) / float(reps * packetLength * 8) * 100.0))
('Numero de pacotes corrompidos: {0:d}'.format(totalPacketErrorCount))
('Taxa de erro de pacotes: {0:.2f}%'.format(float(totalPacketErrorCount) / float(reps) * 100.0))


with open('templateNovo.csv','a',newline='') as f:
    fieldnames =['Número de linhas','Número de colunas','Tamanho do pacote','Numero de transmissoes simuladas','Numero de bits transmitidos','Numero de bits errados inseridos','Taxa de erro de bits (antes da decodificacao)','Numero de bits corrompidos apos decodificacao','Taxa de erro de bits (apos decodificacao)','Numero de pacotes corrompidos','Taxa de erro de pacotes']
